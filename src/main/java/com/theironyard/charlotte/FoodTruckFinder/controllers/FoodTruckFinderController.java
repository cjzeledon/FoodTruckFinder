...
//            }
    ...
    public void addSpecialToFoodTruck(@RequestBody FoodTruckSpecial special, HttpSession session, HttpServletResponse response) throws IOException{
        ...
        if (u.getUserType().equals(UserType.owner) && u.getFoodTruck() != null){
            special.setTruck(u.getFoodTruck());
            specialRepo.save(special);
            userRepo.save(u);

            // if the current user is NOT a food truck owner, he or she will receive an error
        } else {
    ...
    @CrossOrigin
    @PatchMapping("/user/special/edit")
    public void editSpecial(@RequestParam int id, @RequestBody FoodTruckSpecial spec, HttpSession session, HttpServletResponse response) throws IOException{
        // Get the user in the current session
        User u = (User)session.getAttribute(USER_KEY);

        // if the user is an owner and has a food truck assigned to them
        if (u.getUserType().equals(UserType.owner) && u.getFoodTruck() != null){

            // Get a special meal ID
            FoodTruckSpecial editSpecial = specialRepo.findOne(id);

            // Edit the name
            editSpecial.setName(spec.getName());

            // Edit the description
            editSpecial.setDescription(spec.getDescription());

            // Save the new information to the Special (meal) database
            specialRepo.save(editSpecial);
        } else {
            response.sendError(422, "User is either not a food truck owner or does not have an assigned food truck to edit a special meal");
        }
    }

    @CrossOrigin
    @DeleteMapping("/user/special/delete")
    public void deleteSpecial(@RequestParam int id, HttpSession session, HttpServletResponse response) throws IOException{
        // Get the user in the current session
        User u = (User)session.getAttribute(USER_KEY);

        // If the user is an owner and has a food truck assigned to them
        if (u.getUserType().equals(UserType.owner) && u.getFoodTruck() !=null){
            FoodTruckSpecial removeSpecial = specialRepo.findOne(id);
            specialRepo.delete(removeSpecial);
        } else {
            response.sendError(422, "User is either not a food truck owner or does not have an assigned food truck to delete a special meal");
        }
    }

    // Get all the specials currently available in the database
    @CrossOrigin
